...
const setHomeMovieItems = (categoryList) => {
    let container = document.getElementById('homeResultsContainer');
    container.innerHTML='';
    categoryList.forEach( async (category) => {
        ...
        // setChilds(categoryContainer,[categoryMoviesContainer])
        topLineContainer.appendChild(sectionTitle)
        topLineContainer.appendChild(viewAllLink)
        categoryContainer.appendChild(topLineContainer)
        categoryContainer.appendChild(categoryMoviesContainer)
        moviesByCategory = await getCategoryMovieResults(category)
        // console.log(moviesByCategory)
        moviesByCategory.slice(0,5).forEach( movie => {
            ...
            categoryMoviesContainer.appendChild(movieItem)
        })
        categoryContainer.appendChild(categoryMoviesContainer)
    })
}
